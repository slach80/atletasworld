...
- **Task #14 — VALD ForceDecks: finish profile matching + build result sync**: Phase 1 deployed to prod (2026-08-03). Profile matching **complete as of 2026-08-07** — all 33 VALD Hub profiles linked to roster Players (25 auto-matched via `vald_match_profiles`, 8 linked manually via `owner_match_profile` after Mirko's Hub duplicate cleanup — the 8 were roster/Hub name-format mismatches, not missing roster entries: hyphenation, accents, nicknames, spelling variants). Duplicate "Ryan Rivera" roster entries resolved by merging the inactive duplicate Client account into the active one. Remaining work: build the actual Phase 2 sync task (`performance/tasks.py` — pull `/tests` + `/trials`, best-of-trial aggregation, upsert into `ValdTestResult`) — "Sync Now" button is still a stub.
