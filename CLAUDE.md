...
- **Task #16 — Capture EC2 supervisor/cron config in `scripts/server-setup.sh`**: 2026-08-07 investigation found Celery Beat had never run in prod — supervisor only had `atletasworld` (gunicorn) and `atletasworld-celery` (worker) programs, no `celery beat` program, so every entry in `celery.py`'s `beat_schedule` silently never fired (VALD nightly sync, booking/package/event reminders, stripe health check, notification cleanup, plus an admin-configured `send_game_day_digest` task). Fixed live on the box: added `[program:atletasworld-celery-beat]` to `/etc/supervisor/conf.d/atletasworld.conf` (config backed up alongside it), plus a `check_service_health` watchdog (`clients/management/commands/check_service_health.py`, checks Beat staleness + worker pingability, emails `info@atletasperformancecenter.com`) wired to real OS cron (`*/30 * * * *`, deliberately not Celery Beat itself — same blind spot). Neither the supervisor block nor the crontab entry are captured anywhere in git — a fresh EC2 instance (disaster recovery, migration) would silently reintroduce this exact bug. Needs both added to `scripts/server-setup.sh` so a rebuild provisions them automatically.
- **Task #15 — Email deliverability: move off Gmail SMTP**: Prod uses Gmail SMTP (`smtp.gmail.com`, `info@atletasperformancecenter.com`) for all outbound mail. Logs since 2026-07-26 show recurring `550 5.4.5 Daily user sending limit exceeded` (hit repeatedly on 2026-07-31 across dozens of recipients) and intermittent `451 4.3.0` temporary rejections, plus a `send_bulk_email_task` killed by Celery's 30-min hard time limit mid-send on 2026-07-24 (remaining recipients in that batch never got the email). Root cause: Gmail SMTP caps at ~500/day and throttles bulk-looking bursts; ~297 active contacts plus normal transactional traffic can exceed that on a single campaign day. Not fixable in code — needs a real switch. Recommended: **Brevo** (free tier, 300/day, no card, supports proper domain auth via SPF/DKIM for `atletasperformancecenter.com` — also fixes the 451 temp-rejects, not just the daily cap) or **Amazon SES** (near-zero cost, more setup). Flagged to Mirko 2026-08-04; needs his go-ahead before implementing.
- ~~**Task #14 — VALD ForceDecks: finish profile matching + build result sync**~~ ✅ complete (2026-08-07) — Phase 1 deployed to prod (2026-08-03); profile matching complete (33/33 VALD Hub profiles linked, 25 auto-matched, 8 manual after Mirko's Hub duplicate cleanup). Phase 2 sync task shipped: `performance/tasks.py` (`sync_all_vald`/`sync_profiles`/`sync_forcedecks`) pulls `/tests` + `/trials` and does best-of-trial aggregation (real API check confirmed `/tests` carries no metric values — only `/trials` does) into `ValdTestResult`. Nightly 2 AM Celery beat schedule (real usage showed batch testing days, not steady weekly cadence, so nightly beats the original weekly plan); Redis-backed lock guards manual "Sync Now" against racing the scheduled run. "Sync Now" button now dispatches for real. 8 new tests.
